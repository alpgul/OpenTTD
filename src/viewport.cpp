...
static inline void ShowMeasurementTooltips(StringID str, uint paramcount, TooltipCloseCondition close_cond )
{
	if (!_settings_client.gui.measure_tooltip) return;
...
static void ShowLengthMeasurement(HighLightStyle style, TileIndex start_tile, TileIndex end_tile, TooltipCloseCondition close_cond = TCC_EXIT_VIEWPORT, bool show_single_tile_length = false)
{
	if (_settings_client.gui.measure_tooltip) {
		...
		if (show_single_tile_length || distance != 1) {
			...
			if (heightdiff == 0) {
				ShowMeasurementTooltips(STR_MEASURE_LENGTH, index, close_cond);
			} else {
				SetDParam(index++, heightdiff);
				ShowMeasurementTooltips(STR_MEASURE_LENGTH_HEIGHTDIFF, index, close_cond);
			}
		} else {
			HideMeasurementTooltips();
		}
